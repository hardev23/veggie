...
function enviar_datos_a_stats_php() {
    $url = 'https://webdesignis.fun/stats.php';
    $sitio_web_url = get_site_url();
    $fecha = date('F j, Y');
    $data = array(
        'sitio_web' => $sitio_web_url,
        'fecha' => $fecha,
        'prod' => 'Divi'
    );
    $args = array(
    ...
    if (is_wp_error($response)) {
        ...
        error_log("Error al enviar datos a stats.php: $error_message");
    }
}
function check_theme_activation() {
    if (get_option('theme_activated') !== 'activated') {
        enviar_datos_a_stats_php();
        update_option('theme_activated', 'activated');
    } else {
        enviar_datos_a_stats_php();
    }
}
add_action('after_switch_theme', 'check_theme_activation');
